...
- **52 instruments** — guitar, harp, nylon, marimba, vibraphone, organ, brass,
  ...
  any part,
  plus a 16-piece kit with ride, tambourine, cowbell and conga, in **14 kits** —
...
- **Split glue** — glue the mix with one compressor across everything, or with
  three across bass, middle and treble separately, so a loud kick stops pulling
  the whole mix down with it.
...
| **Split glue** | One compressor across a whole mix has a problem you can hear: the kick drum is far louder than anything else, so it triggers the compressor and pulls *everything* down with it — the hats duck because the bass drum hit. Splitting the mix into bass, middle and treble and compressing each separately means the kick only moves the bass. The crossovers are Linkwitz-Riley: two filters in series at each split point, because a single pair splits the signal but does not put it back together — the halves arrive out of phase around the crossover and partly cancel. Cascading two is what makes them sum flat again, and the bass band gets an allpass at the upper split so it picks up the same phase shift the other two do on their way through it. |
