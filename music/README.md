...
- **Select a group of notes and do something to them.** A **Select** tool:
  drag a box round a passage, then move it with the arrow keys (sideways by the
  snap setting, up and down by a semitone, or an octave with Shift), copy and
  paste it with Ctrl+C and Ctrl+V, straighten it onto the grid, or delete it.
  Ctrl+A takes the whole part. **Repeat this bar** copies the bar you are
  looking at into the next one. Everything lands in the undo history.
- **Loop the bars you are working on.** Drag across the bar numbers along the
  top of the grid, or press **Loop these bars**, and playback goes round that
  stretch instead of the whole song while you fix it. Everything outside the
  loop dims, so which bars are live is something you see rather than read.
- **Set how hard each note hits.** A strip under the piano roll, one stem per
  note — drag a stem up or down. On the drum grid it shows one piece at a time
  and follows whichever drum you last touched, so you can shape the hats
...
- **It tells you why there is no sound.** Silence has half a dozen everyday
  causes — a browser waiting for a tap before it will play anything, a muted
  mixer, a part soloed *and* muted at once, the volume down — and the program
  knew all of them and said none of them: the play button flipped to "playing"
  whether or not a single sample ever left the page. Press **🔇 No sound?** at
  any time, or just press play and it will say so by itself.
...
- **Mixer** — mute, solo or rebalance drums, bass, chords, arp, lead, answer
  and pad.
- **Reverb that puts you in a room rather than under a shower.** A gap before
  the first reflection, a handful of discrete early reflections whose pattern is
  what tells your ear the size of the space, and a tail that starts as countable
  echoes and fills into a wash — losing its treble as it goes, the way air does.
  Size changes the shape of the room and not its loudness.
...
| **A selection of live references** | The selection holds the actual note objects out of `song.tracks[…]`, not copies or indices — which is what lets a drag move eight notes by writing straight through them. The cost is that an undo, which swaps in a whole new array, leaves the selection pointing at notes that are no longer in the song, so every path that replaces a track clears it: undo, redo, changing part, changing tool. A stale selection is not a cosmetic bug — the next edit would write to notes nobody can see. |
| **Clamping a group, not its members** | A move that would push any one note past the end of the song is refused outright rather than clamped. Clamping each note where it stands squashes a phrase against the edge: the first note stops, the rest keep going, and the shape you selected is quietly destroyed. Refusing is the only answer that keeps the group a group. |
| **A room, built the way a room is built** | The reverb was white noise multiplied by a fading curve — an honest description of hiss and a poor one of a space, which is why turning it up only ever made things wetter rather than further away. A real impulse response has three parts and now so does this one. **A gap**: sound has to reach a wall and come back, so nothing at all happens for the first 21 ms of the default room (9.8 ms in a small one, 35.6 ms in a large one) — that silence is most of what separates an instrument standing *in* a room from one drowning in it. **Discrete early reflections**: ten taps at the distances six surfaces would give, alternating polarity because real reflections arrive either way up and all-positive taps sum into a thud; the window between the gap and the tail measures 99.8% silence with the taps standing alone in it, and the *pattern* of them is what a listener judges size and shape from. **A tail that fills in**: echo density in a real room grows with the square of time, so the tail is thinned rather than quietened at the start — 0.72 of samples sounding, rising to 1.00 — which is the actual difference between a few countable echoes and a wash. The tail is also built in three bands with separate decay times, because air and soft furnishings eat treble on every bounce: measured, it ends 2.5× duller than it starts, where a single lowpass in front of the convolver made the whole thing equally dull from the first millisecond. Both ears hear the same room and drift apart (correlation 0.26 → −0.02) rather than being two unrelated noises, which is wide but reads as hiss. And the impulse is normalised to a fixed total energy, so size changes the shape of the space and nothing else — a room seven times larger measures 1.00× the loudness, where before every move of that control was half a volume change. |
| **Direct current under the whole mix** | Found by silencing a section and discovering it was not silent: with every note gone from eight bars, the output sat at a steady **−0.075** — no zero crossings in a second, so not sound at all but a constant offset. The cause is a rule that is easy to walk past: Web Audio gives a waveshaper an *endless tail* when its curve is not zero at zero, because such a node keeps producing output for ever, so it is never allowed to go quiet. The drive shaper's curve was built with `x = 2i/n − 1` rather than `2i/(n−1) − 1`, which runs it from −1 to +0.998 instead of −1 to +1 and leaves the point a silent input lands on just below zero. Every note ever played left one of these behind holding the output down. Fixing the divisor removed it entirely — every part's bus now measures 0.0000 during the music and after it, a silenced section renders as literal zeroes, and, unexpectedly, two identical renders came back byte-for-byte the same, where before they had differed by up to 8% per bar. The suite had been blaming that variation on the browser; most of it was this. |
| **An icon that tells the truth about the transport** | The play button used to flip to "playing" the instant it was pressed, regardless of whether playback started. A browser holding audio back until you tap the page, an audio device that would not open, a muted mixer — all of them looked exactly like a working program that had gone quiet, and the one thing the app never did was say so. The transport now reports what actually happened, and a watchdog checks a second later that sound is really coming out of the meter. When it is not, the cause is named: the program can see every one of them, and the only question worth answering is which. Where nothing is wrong that it can see and the meter is still flat, it says the signal is leaving here and stopping somewhere past it, and where to look — a muted tab, a phone on silent, the wrong output. |
| **A loop that is the whole song when it is off** | Looping four bars while you work could have been a second scheduler beside the one that plays songs. Instead the player asks three questions — where the loop starts, where it ends, how long it is — and with no range set they answer 0, the end of the song, and the song's length, which is exactly the arithmetic that was there before. Every other line is unchanged, so there is one code path to get right rather than two that have to agree. |
| **Silence at the flattening, not at the mixer** | Muting a section is applied in the one function that turns a score into a list of notes to play. That function feeds live playback *and* the offline render, so a bridge you silenced to hear the song without it is missing from the exported file too, with neither end being told anything. Doing it at the mixer would have silenced playback and quietly exported the section anyway. |
| **A part shifts by dropping, a selection by refusing** | The two look like the same operation and want opposite rules. A selection that would run past the end is refused whole, because clamping destroys the shape. A *whole part* nudged a sixteenth late would be refused every single time under that rule — the last note of a part almost always ends exactly on the final bar line — so here the tail is trimmed and anything pushed off the end is dropped. The shape being protected in the first case does not exist in the second: there is nothing after the part to squash it against. |
