...
A camera in the 3D look has a **shutter**, like a film camera: whatever moved while it was open lands
on the frame as a smear, and that smear is what joins one frame to the next rather than leaving a pan
looking like a series of separate photographs. It is on in the command-line renderer and off while a
film is playing in the browser, because it costs about a tenth of the frame budget and the browser's
budget has nothing spare. See [docs/FILM_3D.md](../docs/FILM_3D.md) for the measurements.
...
### The engine's people move now

The native renderer was ported by taking the poses and checking them against the
browser to a fraction of a pixel — and it never took the MOTION, so its people
were anatomically correct and completely dead. They held one shape for a whole
shot, never breathed, never looked at whoever they were talking to, and snapped
to a new shape at every cut.

`engine/include/maz/film/Motion.hpp` is the five things that were missing:

- **breathing** — three slow cycles on joints that already exist, so somebody
  standing still is still doing something
- **gaze** — turning toward whoever else is in the scene, the head leading and
  the torso following about a third as far
- **easing at the cut** — blending out of the pose the previous shot left them
  in, derived from the reel rather than remembered between frames, so seeking
  and playback cannot disagree
- **walking** — a stride on the push beats, carrying the figure across part of
  the frame
- **gesturing** — the head and the nearer hand moving on the speaker's own
  syllables, the same clock the score fires its blips on

Checked against a fixture generated by running the browser's own
`film-figures.js` in Node: 504 motion cases and 490 pose choices, agreeing to
1e-12. Not a tolerance — it is the same arithmetic in two languages, and the two
places where that is not automatic (JavaScript's `%` on a double, and
`Math.round` being `floor(x + 0.5)`) are the two places a port drifts silently.

**Porting it found two bugs that had been shipping.**

The native renderer was choosing its poses by a completely different rule —
`hash(name + beat) + seed` rather than the browser's tension-weighted draw — so
the two renderers had been putting the same character in different shapes in the
same shot since the port was written. 303 of 490 cases disagreed. Nothing caught
it because the pose fixtures were captured per pose rather than per shot.

And in the browser, the line that decides which way somebody walks read
`spot.x < 0.5`, where `spot.x` is in world units from 0 to 1000. It was false
for every figure that has ever been drawn, so everybody always walked left
