...
#include "maz/math/SimplifyPolyline.hpp" // simplifyPolyline: defined there, not a second time here
...
// simplifyPolyline (Ramer-Douglas-Peucker) lives in maz/math/SimplifyPolyline.hpp, included above. It
// used to be written out here as well, identically: 28,011 comparisons over random polylines, collinear
// runs, all-duplicate runs and a spike sitting exactly on the tolerance found no input the two disagreed
// on. Two copies of one function under one name is not a spare — a file including both headers did not
// compile. That header also exposes simplifyPolylineIndices, which this copy never had.
