...
// MD5 digest (16 bytes) — Godot's HashingContext HASH_MD5. Legacy: fine for non-security checksums
// (asset dedupe, cache keys, quick change detection) but NOT collision-resistant — use SHA-256 for
// integrity that matters. Standard RFC 1321 algorithm, little-endian, verified against the vectors.
inline std::array<std::uint8_t, 16> md5(const std::uint8_t* data, std::size_t len) {
    ...
    // Padding: 0x80, zeros, then 64-bit LITTLE-endian bit length, to a 64-byte multiple.
    std::vector<std::uint8_t> msg(data, data + len);
    const std::uint64_t bitLen = static_cast<std::uint64_t>(len) * 8u;
    msg.push_back(0x80);
    while (msg.size() % 64 != 56) {
    ...
    for (std::size_t off = 0; off < msg.size(); off += 64) {
        ...
        for (std::size_t i = 0; i < 16; ++i) {
            ...
            m[i] = static_cast<std::uint32_t>(msg[j]) |
                   (static_cast<std::uint32_t>(msg[j + 1]) << 8) |
                   (static_cast<std::uint32_t>(msg[j + 2]) << 16) |
                   (static_cast<std::uint32_t>(msg[j + 3]) << 24);
        }
        ...
        for (int i = 0; i < 64; ++i) {
            ...
            f = f + a + K[i] + m[static_cast<std::size_t>(g)];
            a = d;
            d = c;
            c = b;
            b = b + rotl(f, S[i]);
    ...
    const std::uint32_t hs[4] = {a0, b0, c0, d0};
    for (int i = 0; i < 4; ++i) { // little-endian output
        out[static_cast<std::size_t>(i * 4)] = static_cast<std::uint8_t>(hs[i] & 0xFF);
        out[static_cast<std::size_t>(i * 4 + 1)] = static_cast<std::uint8_t>((hs[i] >> 8) & 0xFF);
        out[static_cast<std::size_t>(i * 4 + 2)] = static_cast<std::uint8_t>((hs[i] >> 16) & 0xFF);
        out[static_cast<std::size_t>(i * 4 + 3)] = static_cast<std::uint8_t>((hs[i] >> 24) & 0xFF);
    }
    return out;
}
inline std::array<std::uint8_t, 16> md5(const std::string& s) {
    return md5(reinterpret_cast<const std::uint8_t*>(s.data()), s.size());
}
