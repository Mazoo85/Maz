...
// The generated Android `settings.gradle`: names the Gradle project after the app. The SDL3 Android module
// (org.libsdl.app.SDLActivity, referenced by the manifest) is added by the owner — a one-line include here.
inline std::string androidSettingsGradle(const std::string& appName) {
    return "rootProject.name = '" + appIdSegment(appName) + "'\n"
           "// include ':SDL3'   // add the SDL3 Android module/aar that provides org.libsdl.app.SDLActivity\n";
}

// The generated Android app `build.gradle`. mobilepack pre-stages the native game library and assets
// (lib/<abi>/lib<App>.so + assets/, via io::planMobileBundle), so this Gradle module does NOT compile native
// code — it just packages the prebuilt `.so`s (jniLibs) and assets into a signed APK. `abis` becomes the
// ndk abiFilters so the APK ships only the ABIs that were staged. Pure text, exactly like the manifest.
inline std::string androidBuildGradle(const std::string& appName, const std::string& version,
                                      const std::vector<std::string>& abis, int minSdk = 26,
                                      int targetSdk = 34) {
    std::string filters;
    for (std::size_t i = 0; i < abis.size(); ++i) {
        filters += (i ? ", " : "") + std::string("'") + abis[i] + "'";
    }
    if (filters.empty()) {
        filters = "'arm64-v8a'";
    }
    const std::string id = bundleId(appName);
    return "// Generated by io::androidBuildGradle — packages mobilepack's PRE-STAGED native libs + assets.\n"
           "// No native build here: the game .so per ABI and the assets/ tree are already staged next to\n"
           "// this file (see io::planMobileBundle). Add the SDL3 Android module in settings.gradle.\n"
           "plugins { id 'com.android.application' }\n"
           "\n"
           "android {\n"
           "    namespace '" + id + "'\n"
           "    compileSdk " + std::to_string(targetSdk) + "\n"
           "    defaultConfig {\n"
           "        applicationId \"" + id + "\"\n"
           "        minSdk " + std::to_string(minSdk) + "\n"
           "        targetSdk " + std::to_string(targetSdk) + "\n"
           "        versionCode 1\n"
           "        versionName \"" + version + "\"\n"
           ...
           "            jniLibs.srcDirs = ['lib']   // lib/<abi>/lib" + appName + ".so (prebuilt)\n"
           "            assets.srcDirs = ['assets'] // res:// tree (or a single game.pck)\n"
           "        }\n"
           "    }\n"
           ...
           "\n"
           "dependencies {\n"
           "    // implementation project(':SDL3')   // the module providing org.libsdl.app.SDLActivity\n"
           "}\n";
}
