...
static void writeProjectTo(std::ostream& f, Sequencer& seq, Mixer& mixer, Automation& automation) {
    ...
    f << "songmode " << (seq.songMode() ? 1 : 0) << " " << (seq.songLoop() ? 1 : 0) << " "
      ...
      << " " << seq.clipLoopStart() << " " << seq.clipLoopEnd() << "\n";
...
static bool readProjectFrom(std::istream& f, Sequencer& seq, Mixer& mixer, Automation& automation,
    ...
    while (std::getline(f, line)) {
        ...
        if (tag == "cjc") {
            ...
            int clStart = 0, clEnd = 0; // clip loop region optional (older files omit it → whole timeline)
            if (ls >> clStart >> clEnd) {
                seq.setClipLoopRange(clStart, clEnd);
            }
            ...
            ls >> pat >> bar >> trk;
            int bars = 1; // optional trailing span (older files omit it → single-bar clip)
            if (!(ls >> bars)) {
                bars = 1;
            }
            ...
            int pan = 0; // optional per-step pan (older files omit it → centred)
            if (ls >> pan) {
                seq.setStepPan(c, s, pan);
            }
        } else if (tag == "note") {
            int p = 0;
            Note n;
