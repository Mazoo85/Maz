...
Mixer::Mixer() {
    ...
    // Signal order: gate → EQ → tone → drive → crush → dynamics → modulation → time fx → width →
    // plugins.
    chain_ = {&gate_,   &peq_,    &eq_,     &dist_,     &crush_,  &comp_,  &chorus_,
              &phaser_, &delay_,  &reverb_, &widener_,  &plugin_, &clap_,  &vst3_};
