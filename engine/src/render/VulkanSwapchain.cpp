...
bool VulkanSwapchain::create(VulkanContext& ctx, uint32_t width, uint32_t height, bool vsync) {
    ...
    return createColorTarget(ctx) && createDepthResources(ctx) && createRenderPass(ctx) &&
...
bool VulkanSwapchain::createRenderPass(VulkanContext& ctx) {
    ...
    // resolved into the swapchain image and then discarded), otherwise it is the swapchain image
    // itself and is presented directly.
    ...
    // Attachment 2 (MSAA only): the single-sample swapchain image the color attachment resolves
    // into for presentation.
...
bool VulkanSwapchain::createFramebuffers(VulkanContext& ctx) {
    ...
    for (size_t i = 0; i < m_views.size(); ++i) {
        // Attachment order must match the render pass: [color, depth] single-sample, or
        // [msaa-color, depth, resolve=swapchain] when multisampled.
        VkImageView msaaAttachments[] = {m_colorView, m_depthView, m_views[i]};
        VkImageView singleAttachments[] = {m_views[i], m_depthView};
        VkFramebufferCreateInfo fb{};
        fb.sType = VK_STRUCTURE_TYPE_FRAMEBUFFER_CREATE_INFO;
        fb.renderPass = m_renderPass;
        fb.attachmentCount = msaa ? 3u : 2u;
        fb.pAttachments = msaa ? msaaAttachments : singleAttachments;
