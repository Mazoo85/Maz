...
- [~] Containers: `small_vector`, sparse set, ring buffer — **`maz::core::RingBuffer<T>` landed**
      (fixed-capacity FIFO ring/circular queue à la Godot's `RingBuffer`; header-only, unit-tested);
      `small_vector` + sparse set still TODO
