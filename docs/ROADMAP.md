...
- [~] Transform helpers, AABB/OBB, ray, plane, frustum — **Aabb + Ray + Plane + intersections + Frustum culling + affine Transform landed** (`maz::math::Geometry`/`Frustum`/`Transform`, unit-tested); **`maz::math` quaternion rotation helpers landed** (`Rotation.hpp`: `fromAxisAngle`/`fromEuler` (documented YXZ, Godot-default)/`rotate`/`angleBetween` (unsigned, double-cover-safe)/`slerp` (shortest-path)/`rotateTowards` (angle-clamped step)/`lookRotation` (local -Z→forward, camera convention), header-only, unit-tested); **`maz::math` half-float (IEEE 754 binary16) conversion landed** (`HalfFloat.hpp`: `floatToHalf` f32→f16 round-to-nearest-even with overflow-to-Inf, subnormal-half handling and NaN/Inf passthrough, `halfToFloat` exact f16→f32 decode, via `std::bit_cast`; for GPU vertex/texture compression and HDR storage, the Godot half-float analog; header-only, unit-tested); **`maz::math::Transform2D` landed** (`Transform2D.hpp`: 2D affine transform — a 2x2 basis + translation origin, the map x→basis·x+origin, column-major; closed under compose (`operator*`)/`inverse` incl. non-uniform scale+rotation; `identity`/`translation`/`rotation` (CCW radians)/`scale` factories, `xform` (point) vs `xformBasis` (direction); the Godot Transform2D analog complementing iter5's 3D Transform; header-only, unit-tested); **`maz::math::Rect2` landed** (`Rect2.hpp`: 2D axis-aligned rectangle stored as position (min corner) + size, the Godot Rect2 analog and 2D counterpart to iter2's 3D Aabb; min/max/center/area/hasArea getters, HALF-OPEN `contains` (inclusive min edge, exclusive max edge — has_point convention), `intersects` with EXCLUSIVE borders (edge-touching rects do NOT overlap), `intersection` (overlap rect, zero-size when disjoint), `merge`/`expand` bounding rect over another rect / a point, `grow` inflating all four sides, `fromMinMax`; header-only, unit-tested); OBB TODO
...
- [~] 3D collision shapes, raycasts / queries, triggers / overlaps — **Sphere primitive + sphere/ray-sphere + ray-triangle intersection tests landed** (`maz::math::Collision`: `Sphere` + `sphereSphere`/`sphereAabb`/`spherePlane`/`raySphere`/`closestPointOnAabb`, plus `rayTriangle` (Möller–Trumbore, barycentrics + optional backface cull) for mesh picking / raycasting, complementing iter2 Geometry Aabb/Ray/Plane; unit-tested); **2D convex hull landed** (`maz::math::convexHull2D`: Andrew's monotone chain returning CCW hull vertices with collinear edge points excluded (minimal hull) and no repeated closing vertex, deterministic O(n log n), the Godot collision-shape / convex-hull analog; header-only, unit-tested); **2D convex-polygon overlap landed** (`maz::math::convexPolygonsOverlap`: Separating Axis Theorem over both polygons' edge normals with strict-disjoint (inclusive touching) interval tests, unnormalized/scale-invariant axes, any winding, pairs with convexHull2D; header-only, unit-tested); **2D point-in-polygon landed** (`maz::math::pointInPolygon`: crossing-number (even-odd) ray-casting containment test that — unlike convexPolygonsOverlap — handles ANY simple polygon (convex OR concave, any winding), boundary result unspecified, O(n); header-only, unit-tested); **2D segment-segment intersection landed** (`maz::math::segmentIntersection`: line-segment vs line-segment single-point crossing test — writes the crossing point and returns true when two segments meet at one point, returns false for parallel/collinear (even overlapping)/out-of-span pairs, inclusive endpoints (shared endpoint / T-junction count), the Godot Geometry2D.segment_intersects_segment analog; header-only, unit-tested); full collision shapes/queries/triggers TODO
