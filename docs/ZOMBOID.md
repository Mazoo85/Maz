...
| 11 | **Screamer** | Fragile back-line support that **holds its distance** — it backs away as you close in rather than shambling into melee, so you must push through the horde or pick it off at range. Periodically **shrieks**, whipping nearby zombies into a speed frenzy (the **boss is immune** — it can't be sped up beyond its own enrage). It **winds up with a tell first**, so you get a beat to react: burst it down, **chill it**, or **stagger it** (a melee shove or dash-strike) during the wind-up and the shriek is cut off entirely. Silence it first. |
| 12 | **Healer** | A back-line medic that **holds its distance** (backing away as you approach, like the summoner) and periodically **mends** nearby wounded zombies, undoing your chip damage. It **winds up with a tell first**, so you get a beat to kill it, **chill it**, or **stagger it** (a melee shove or dash-strike) during the wind-up to cancel the mend. Fragile and never heals itself — and it **can't mend the boss** either, so it won't refund a wave leader's health bar. It also **can't patch a burning, bleeding, *or* frozen body**: damage-over-time (and cold) holds a wound open against all healing, so setting the pack alight (molotov / flamethrower), lacerating it, **or freezing it** (Cryo Nova / Frost Field) denies the mend just as it denies a Regenerator's self-heal — one unified anti-heal rule across every heal source. Cull it before it patches the rest of the pack back up. |
