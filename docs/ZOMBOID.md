...
| 3 | **Boss** | Huge bullet-sponge that leads every 5th wave; **ground-slams** for heavy damage *and* hurls you back — but it **flashes a warning as it winds up**, so dash clear of the ring to dodge it. A **dodge-roll's i-frames ride the slam out completely** — no damage *and* no knockback, just like a brute's blow — so a well-timed roll is a full-body dodge, not just a damage skip. **Enrages** below 35% health — faster, slams twice as often, calls in waves of runners, and **slowly heals itself** through the rage, so the climax rewards sustained pressure. That self-heal obeys the same rule as every other: a **burning, bleeding, or chilled boss can't heal**, so keep fire, laceration, or cold on it during the enrage and it can't claw its health back. Felling one always drops a **full care package — a guaranteed medkit *and* a guaranteed power-up** — so grinding the wave leader down pays off big. While a boss is alive a **dedicated health bar appears across the bottom of the screen** (it turns a pulsing red and reads *ENRAGED* once the boss drops below 35%), so you can gauge exactly how close the fight is to the finish. And if you kite far enough that the boss scrolls off-screen, a **pulsing magenta marker pins to the screen edge in its direction** so you never lose track of where the wave leader is. |
...
| 5 | **Spitter** | Hangs back and lobs acid — and it **holds its firing distance**, backpedalling to reopen a gap if you rush it, so you have to corner it, out-range it, or freeze it rather than just strolling up. Its acid leaves a caustic **puddle** on the ground — it eats your health *and* **slows you to half speed** while you stand in it, so slog clear rather than tanking it. **Chill it or stagger it and it can't lob** — a Cryo Nova / Frost Field silences it like the casters, and a melee shove or dash-strike roots the throw. But the sludge **corrodes the horde too**: any zombie wading through it gets **bogged down** (slowed), so a spitter's own puddle is a double-edged zone — **kite the swarm through it** to slow the pack (torch it to actually hurt them). |
...
| 7 | **Summoner** | A back-line necromancer that **keeps its distance** — retreats when you close in while it calls reinforcements. Early on it calls fodder **walkers**, but from the mid-game (wave 5+) it calls faster **runners**, so a summoner left alive stays a real threat deep into a run. Each call **winds up with a tell first** — its body **flashes cyan** as it charges — so you get a beat to burst it, **chill it**, or **stagger it** (a melee shove or dash-strike) to cancel the summon mid-cast. Chase it down. |
...
| 11 | **Screamer** | Fragile back-line support that **holds its distance** — it backs away as you close in rather than shambling into melee, so you must push through the horde or pick it off at range. Periodically **shrieks**, whipping nearby zombies into a speed frenzy (the **boss is immune** — it can't be sped up beyond its own enrage). It **winds up with a tell first** — its body **flashes magenta** as it charges — so you get a beat to react: burst it down, **chill it**, or **stagger it** (a melee shove or dash-strike) during the wind-up and the shriek is cut off entirely. Silence it first. |
| 12 | **Healer** | A back-line medic that **holds its distance** (backing away as you approach, like the summoner) and periodically **mends** nearby wounded zombies, undoing your chip damage. It **winds up with a tell first** — its body **flashes green** as it charges — so you get a beat to kill it, **chill it**, or **stagger it** (a melee shove or dash-strike) during the wind-up to cancel the mend. Fragile and never heals itself — and it **can't mend the boss** either, so it won't refund a wave leader's health bar. It also **can't patch a burning, bleeding, *or* frozen body**: damage-over-time (and cold) holds a wound open against all healing, so setting the pack alight (molotov / flamethrower), lacerating it, **or freezing it** (Cryo Nova / Frost Field) denies the mend just as it denies a Regenerator's self-heal — one unified anti-heal rule across every heal source. Cull it before it patches the rest of the pack back up. |
