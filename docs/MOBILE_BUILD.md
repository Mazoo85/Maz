...
| Android Gradle project | `io::androidBuildGradle`/`androidSettingsGradle` — mobilepack drops a `build.gradle` + `settings.gradle` that package the prebuilt `jniLibs` + assets (abiFilters, reverse-DNS id), so the staged tree is `gradle assembleRelease`-ready | ✅ done |
...
1. **Project skeleton.** `mobilepack` already drops a generated `build.gradle` + `settings.gradle` into the
   staged Android bundle (via `io::androidBuildGradle`) — a prebuilt-packaging module that ships the staged
   `lib/<abi>/*.so` (jniLibs) and `assets/` with the right applicationId/abiFilters/SDK levels, so you don't
   hand-author the Gradle project; just add the SDL3 Android module (`settings.gradle` has the commented
   `include ':SDL3'`) from SDL3's template (`SDL/android-project`). Build Maz + your game as a shared library
   loaded by `SDLActivity`. SDL provides the `android_app`/`ANativeWindow` and pumps touch/lifecycle events
   into the same `SDL_Event` path `Window::pumpEvents` already handles — so touch "just works" once the
   window exists.
...
- The iOS Xcode project generation and signing config (the Android `build.gradle`/`settings.gradle` are now
  generated by `mobilepack` — see above; signing config is still yours).
