...
- **Undo a night:** every live run works on its own `forge/YYYY-MM-DD-<slug>`
  branch and opens a *draft* pull request. Close the PR and delete the branch.
  Nothing the Forge does is ever merged without you — it has no ability to
...
| **DO** | Refuses to start on a dirty working tree; otherwise creates a branch and hands the single task to Maz Crew |
| **VERIFY** | Runs the zone's checks; green pushes the branch and opens a draft PR, red abandons the branch |
...
| `safe_zones` | `docs/`, `madlibs/`, `music/`, `shooter/`, `scraper/`, `crew/tests/`, `tests/` | Where it may work |
...
| `pr_opened` | Checks were green, the branch pushed, and a draft PR was opened |
...
| `crew_failed` | Crew crashed, timed out, touched a no-touch or out-of-zone path, or the working tree was dirty when the run tried to start |
| `budget_exceeded` | Crew reported a cost over `budget_usd` (only possible with a cost-reporting runner — see above) |

A run always refuses to start if the working tree is not clean (no
uncommitted changes) — that check runs before anything is created, so a
human's in-progress edits are never mistaken for the Forge's own and never
end up on a Forge branch.
