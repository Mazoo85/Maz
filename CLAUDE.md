...
which of them an app demonstrates, and a ranked queue of what is missing. It is kept current while
you work — the session-start hook runs `--watch` — so read it rather than re-deriving it, and run
`node tools/inventory/main.mjs --status` if you want to know which tree it describes and how far
behind the trunk you are. (There is already a `math::Geometry2D`,
...
`node tools/inventory/main.mjs --status` prints that number among other things, and says so in
words when it is not zero.

**Other sessions are working in this repo at the same time as you.** Merge the trunk before trusting
any comparison against it, and expect it to have moved: a single session's work here has been
overtaken by 29 trunk commits more than once. Two generated files —`docs/INVENTORY.md` and
`docs/inventory.json` — would conflict on every such merge, so `.gitattributes` names a merge driver
that resolves them by rescanning the merged tree instead. The session-start hook registers it; if
you ever do see a conflict in either, do not hand-merge it, run
