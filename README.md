...
- **Instruments** — a synthesized drum kit (`DrumVoice` — kick/snare/hats/clap/**tom**/**cowbell**/**rimshot**/**crash cymbal**, **per-channel selectable** per row, per-hit velocity, **choke groups**, per-channel **tuning** + **decay** + **drive/saturation** + **flam**); **two** polyphonic
  ...
  noise + a selectable-waveform, **-1/-2 octave sub** + **unison** supersaw stack + a resonant state-variable **filter** — **low-pass/high-pass/band-pass** selectable — with envelope + **velocity sensitivity** + **key tracking** + a **cutoff LFO** (wobble/auto-wah)), **FM** (with operator **feedback**), and **wavetable** (a morphing
...
- **Modulation** — automation of master-EQ-cutoff/FM/reverb/master/delay-mix/distortion-drive/stereo-width/**synth-filter-cutoff**/**synth-filter-resonance** (the classic lead sweep + acid Q builds), synced to the transport, from either an
