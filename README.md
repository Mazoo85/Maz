...
- **Instruments** — a synthesized drum kit (`DrumVoice` — kick/snare/hats/clap/**tom**/**cowbell**/**rimshot**/**crash cymbal**, **per-channel selectable** per row, per-hit velocity, **choke groups**, per-channel **tuning** + **decay** + **drive/saturation** + **flam**); **two** polyphonic
  ...
  noise + a selectable-waveform, **-1/-2 octave sub** + **unison** supersaw stack + a resonant state-variable **filter** — **low-pass/high-pass/band-pass** selectable — with envelope + **velocity sensitivity** + **key tracking** + a **cutoff LFO** (wobble/auto-wah) + a **dedicated filter envelope** (its own ADSR sweeping the cutoff, bipolar depth in Hz)), **FM** (with operator **feedback**), and **wavetable** (a morphing
...
- **Mixing** — per-channel volume/mute/solo/**pan**, **pan for the lead & bass buses**, true **stereo**, **per-bus insert strips**
  ...
  compressor (soft-knee, parallel/dry-wet mix), transient shaper (attack/sustain designer), de-esser (frequency-selective high-band compression), chorus, flanger, and phaser (all three **tempo-syncable**), delay (with ping-pong + damping + **tempo sync** to note divisions), stereo/dual delay (independent L/R times, each **tempo-syncable** to its own note division), reverb (with pre-delay + width + freeze + ducking), mid/side stereo
...
- **Modulation** — automation of master-EQ-cutoff/FM/reverb/master/delay-mix/distortion-drive/stereo-width/**synth-filter-cutoff**/**synth-filter-resonance**/**lead-bus volume**/**lead-bus pan** (the classic lead sweep + acid Q builds + bus volume rides + auto-pan), synced to the transport, from either an
