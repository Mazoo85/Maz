...
- **Instruments** — a synthesized drum kit (`DrumVoice` — kick/snare/hats/clap/**tom**/**cowbell**/**rimshot**/**crash cymbal**/**ride cymbal**/**shaker**/**clave**/**tambourine**/**conga**/**woodblock**/**bongo**/**triangle**/**808** (a long, pitch-gliding pure sub kick)/**zap** (a synthwave laser "pew" — a fast, wide downward pitch sweep), **per-channel selectable** per row, per-hit velocity, **choke groups**, per-channel **tuning** + **decay** + **drive/saturation** + **flam** + **pitch-envelope depth / "punch"** + **pitch-envelope time** (together shape the kick/tom pitch sweep — flat sub → snappy click → long 808 boom) + **tone** (a per-drum low-pass to darken bright hits, FL channel-filter style) + a snare **snap** (noise-vs-tone balance, from tom-like body to a crisp wire crack)); **two** polyphonic
  ...
  single-cycle table with **selectable frames**, scanned by an envelope- **and LFO**- **and velocity**-sweepable position), and **pluck** (a Karplus-Strong physical-modelled plucked string — a noise burst fed through a tuned, damped delay line for natural guitar/harp/koto-style plucks, with a **damping** control from bright/long-ringing to soft/muted) engines, plus **portamento/glide** (with a **legato-only** mode — glide just on overlapping notes), **analog drift** (per-note detune for analog warmth), **start-phase randomization** (each note begins its oscillators at a random phase for analog-style variation), **vibrato** (with an **onset delay** that fades it in on sustained notes), **tremolo** (amplitude LFO), a **pitch envelope**, **oscillator hard sync**, **pulse width (PWM)** with a **PWM LFO** (lush sweeping duty cycle), a per-instrument **octave** shift, and a **mono** (single-voice) mode;
