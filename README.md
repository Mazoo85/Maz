...
- **Instruments** — a synthesized drum kit (`DrumVoice` — kick/snare/hats/clap/**tom**/**cowbell**/**rimshot**/**crash cymbal**/**ride cymbal**/**shaker**/**clave**/**tambourine**/**conga**, **per-channel selectable** per row, per-hit velocity, **choke groups**, per-channel **tuning** + **decay** + **drive/saturation** + **flam** + **pitch-envelope depth / "punch"** + **pitch-envelope time** (together shape the kick/tom pitch sweep — flat sub → snappy click → long 808 boom) + **tone** (a per-drum low-pass to darken bright hits, FL channel-filter style)); **two** polyphonic
  synths (lead + bass, `SynthInstrument`) with **subtractive** (a **three-oscillator** stack — five selectable shapes per oscillator (sine / square / saw / triangle / **trapezoid** — a clipped triangle, warmer than a square yet brighter than a triangle) + detuned 2nd osc + **coarse-tunable** 2nd & 3rd oscs + **ring mod** + sub +
...
- **Mixing** — per-channel volume/mute/solo/**pan**, **pan for the lead & bass buses**, true **stereo**, **per-bus insert strips**
  ...
  chain (parametric EQ (low shelf + **two** independent sweepable mid bells + high shelf), tilt EQ, aural exciter (high-band harmonic enhancer), low-pass, high-pass, distortion with selectable curves (soft/hard/fold/sine/**tube** — asymmetric, even-harmonic warmth) + a post **tone** low-pass + an **output-level** trim (drive hard, then gain-stage the result back down), tape saturation (with **wow & flutter** pitch wobble), ring modulator, bitcrusher, noise gate (with hold),
  compressor (soft-knee, parallel/dry-wet mix, **sidechain high-pass** so lows don't pump it, live **gain-reduction meter**), 3-band multiband compressor (Maximus-style — independent low/mid/high threshold + ratio over two crossovers, exact-reconstruction band split), transient shaper (attack/sustain designer), de-esser (frequency-selective high-band compression), chorus (with **feedback** for a deeper, flanger-edged voice), flanger, and phaser (all three **tempo-syncable**; the phaser has a selectable **stage count** for deeper sweeps), delay (with ping-pong + damping + a feedback **low-cut** for dub/analog echoes + **modulation** for warm BBD/tape pitch wobble + **tempo sync** to note divisions), stereo/dual delay (independent L/R times, each **tempo-syncable** to its own note division, with feedback **damping** + **low-cut** tone), reverb (with pre-delay + width + freeze + ducking + wet-tail low-cut/high-cut tone + a **gated** mode for the classic 80s cut-off tail), mid/side stereo
