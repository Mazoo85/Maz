...
- **Mixing** — per-channel volume/mute/solo/**pan**, **pan for the lead & bass buses**, true **stereo**, **per-bus insert strips**
  ...
  chain (parametric EQ, tilt EQ, aural exciter (high-band harmonic enhancer), low-pass, high-pass, distortion with selectable curves (soft/hard/fold/sine/**tube** — asymmetric, even-harmonic warmth) + a post **tone** low-pass + an **output-level** trim (drive hard, then gain-stage the result back down), tape saturation (with **wow & flutter** pitch wobble), ring modulator, bitcrusher, noise gate (with hold),
