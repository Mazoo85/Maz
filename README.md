...
`swarm`, `cube`, `scene3d`, `world`, `model`).
...
The sample model `assets/models/house.gltf` is generated by `tools/make_house_gltf.py` (regenerate
with `python3 tools/make_house_gltf.py`) — a self-contained glTF 2.0 file authored in-repo, with
its geometry *and* a base-color detail texture (a brick/shingle/plank/glass atlas) base64-embedded
in one buffer, so there are no third-party model assets to license.
