...
- **Mixing** — per-channel volume/mute/solo/**pan**, **pan for the lead & bass buses**, true **stereo**, **per-bus insert strips**
  ...
  compressor (soft-knee, parallel/dry-wet mix), transient shaper (attack/sustain designer), de-esser (frequency-selective high-band compression), chorus, flanger, phaser, delay (with ping-pong + damping + **tempo sync** to note divisions), stereo/dual delay (independent L/R times), reverb (with pre-delay + width + freeze), mid/side stereo
