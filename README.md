...
- **Sequencing** — a `Sequencer` channel rack (step grid with per-step velocity, **probability**, and **ratcheting**), **two** `PianoRoll`
  ...
  an **arpeggiator** (up/down/up-down/random/as-played, 1–4 octave range, adjustable **gate length** for staccato/legato, and a **rate** of 1–8 steps per note to run slower than the grid), a **global transpose**, a **metronome** (with an adjustable **level**) with **count-in**, and **sidechain** ducking (with a selectable trigger channel, plus **attack** and release times for an instant hard pump or a softer rounded duck).
- **Mixing** — per-channel volume/mute/solo/**pan**, **pan for the lead & bass buses**, true **stereo**, **per-bus insert strips**
  (drums / lead / bass, each with its own **noise gate** + high-pass + **transient shaper** (per-bus attack/sustain punch) + EQ + distortion + compressor + gain/pan/mute/**solo**), a master effect
