...
- **Instruments** — a synthesized drum kit (`DrumVoice`, per-hit velocity, **choke groups**, per-channel **tuning** + **decay** + **drive/saturation** + **flam**); **two** polyphonic
  ...
  noise + a selectable-waveform, **-1/-2 octave sub** + **unison** supersaw stack + a resonant state-variable **filter** with envelope + **velocity sensitivity**), **FM** (with operator **feedback**), and **wavetable** (a morphing
