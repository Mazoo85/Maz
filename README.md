...
- **Instruments** — a synthesized drum kit (`DrumVoice` — kick/snare/hats/clap/**tom**/**cowbell**/**rimshot**/**crash cymbal**, **per-channel selectable** per row, per-hit velocity, **choke groups**, per-channel **tuning** + **decay** + **drive/saturation** + **flam**); **two** polyphonic
  ...
  single-cycle table with **selectable frames**, scanned by an envelope- **and LFO**-sweepable position) engines, plus **portamento/glide**, **vibrato**, **tremolo** (amplitude LFO), a **pitch envelope**, **oscillator hard sync**, **pulse width (PWM)**, a per-instrument **octave** shift, and a **mono** (single-voice) mode;
