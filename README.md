...
- **Instruments** — a synthesized drum kit (`DrumVoice` — kick/snare/hats/clap/**tom**/**cowbell**/**rimshot**, **per-channel selectable** per row, per-hit velocity, **choke groups**, per-channel **tuning** + **decay** + **drive/saturation** + **flam**); **two** polyphonic
  ...
  single-cycle table with **selectable frames**, scanned by an envelope- **and LFO**-sweepable position) engines, plus **portamento/glide**, **vibrato**, a **pitch envelope**, **oscillator hard sync**, **pulse width (PWM)**, a per-instrument **octave** shift, and a **mono** (single-voice) mode;
  and a **`Sampler`** (WAV playback, pitch-shifted per note, with **reverse**, **loop** (incl. **ping-pong** bounce), **start-offset**, an **amp envelope**, **fine tune**, **normalize**, and **fade in/out**).
- **Sequencing** — a `Sequencer` channel rack (step grid with per-step velocity, **probability**, and **ratcheting**), **two** `PianoRoll`
  lanes (lead + bass) with a **chord tool** (maj/min/7ths/sus/6ths/9ths), **per-note probability**, **quantize** (adjustable strength), **scale-snap** (snap notes to major/minor/modes/pentatonic/blues), **strum** (roll stacked chords), **legato** (glue notes to the next), **invert** (mirror the melody around a pivot), **reverse** (flip the rhythm in time), **humanize** (randomize velocity + timing), and **duplicate** (repeat the phrase), per-channel **step-row rotate** (shift a groove around the bar) and **Euclidean fill** (evenly distributed pulses), a **resizable pattern length** (8–64 steps) with adjustable **steps-per-beat** (16ths, triplets, 32nds), **multiple named + clonable patterns + a playlist** (looping or play-once) to build songs, **swing**, **humanize**,
  an **arpeggiator** (up/down/up-down, 1–4 octave range), a **global transpose**, a **metronome** with **count-in**, and **sidechain** ducking (with a selectable trigger channel).
- **Mixing** — per-channel volume/mute/solo/**pan**, **pan for the lead & bass buses**, true **stereo**, **per-bus insert strips**
  ...
  compressor (soft-knee, parallel/dry-wet mix), transient shaper (attack/sustain designer), chorus, flanger, phaser, delay (with ping-pong + damping), stereo/dual delay (independent L/R times), reverb (with pre-delay + width + freeze), mid/side stereo
  widener, mono-bass maker, auto-pan, envelope filter / auto-wah, comb resonator, tremolo / trance-gate, formant (vowel) filter, utility (gain/phase-invert/mono)),
