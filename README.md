...
- **Sequencing** — a `Sequencer` channel rack (step grid with per-step velocity, **probability**, and **ratcheting**), **two** `PianoRoll`
  lanes (lead + bass), a **resizable pattern length** (8–64 steps) with adjustable **steps-per-beat** (16ths, triplets, 32nds), **multiple patterns + a playlist** to build songs, **swing**, **humanize**,
...
- **Mixing** — per-channel volume/mute/solo/**pan**, true **stereo**, **per-bus insert strips**
  ...
  chain (parametric EQ, low-pass, high-pass, distortion, tape saturation, bitcrusher, noise gate,
