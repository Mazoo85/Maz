// Maz Engine — "ANIMATOR" (animation controller demo)
// A skinned tube is driven by maz::anim::Animator, which holds three named clips (idle / wave /
// coil) and auto-switches between them on a timer with smooth cross-fades. Unlike the raw animclip
// demo (continuous blend of two fixed clips), this shows a stateful controller: play(name, fade)
// transitions like a real character animation graph would. HUD shows the active clip + fade.
...
// A clip of per-joint Z-rotation keyframes forming a travelling wave at the given amplitude.
anim::AnimClip makeWave(float amp) {
    anim::AnimClip c;
    c.duration = 2.0f;
    c.loop = true;
    ...
    for (int i = 0; i < kJoints; ++i) {
        for (float tk : times) {
            const float ang = amp * std::sin(tk * 3.14159265f - static_cast<float>(i) * 0.7f);
...
anim::AnimClip makeCoil() {
    ...
    for (int i = 0; i < kJoints; ++i) {
        c.tracks[static_cast<size_t>(i)].rotation = {
            {0.0f, glm::angleAxis(0.15f, math::vec3(0, 0, 1))},
            {1.0f, glm::angleAxis(0.5f, math::vec3(0, 0, 1))},
            {2.0f, glm::angleAxis(0.15f, math::vec3(0, 0, 1))}};
    }
    return c;
}
...
int main(int argc, char** argv) {
    ...
    MAZ_LOG_INFO("ANIMATOR (animation controller demo) starting");

    platform::Window window;
    platform::WindowConfig wc;
    wc.title = "Maz Engine — Animation Controller";
    wc.width = cfg.width;
    ...
    // Bind mesh (tapered tube, two-bone weights) — same as the skeleton/animclip demos.
    ...
    // The controller: three named clips it cross-fades between on a timer.
    anim::Animator animator;
    animator.setRestPose(rest);
    const char* clipNames[3] = {"idle", "wave", "coil"};
    animator.addClip(clipNames[0], makeWave(0.08f)); // idle: tiny sway
    animator.addClip(clipNames[1], makeWave(0.35f)); // wave: big travelling ripple
    animator.addClip(clipNames[2], makeCoil());      // coil: curl
    ...
    while (!window.shouldClose()) {
        ...
        while (clock.consumeFixedStep()) {
            ...
            t += dt;
            switchTimer += dt;
            // Every 3s, the controller cross-fades to the next clip (0.7s fade).
            if (switchTimer >= 3.0f) {
                switchTimer -= 3.0f;
                clipIdx = (clipIdx + 1) % 3;
                animator.play(clipIdx, 0.7f);
            }
            animator.update(dt);
        }

        anim::posesToLocals(animator.pose(), locals);
        ...
        renderer->setClearColor(render::Color{0.10f, 0.13f, 0.15f, 1.0f});
        if (renderer->beginFrame()) {
            ...
            const float boneCol[4] = {0.4f, 1.0f, 0.7f, 1.0f};
            for (int i = 1; i < kJoints; ++i) {
                const math::vec4 a =
            ...
            font.drawText(*renderer, 16.0f, 12.0f, "MAZ ENGINE  -  ANIMATION CONTROLLER",
                          render::Color{1, 1, 1, 1}, 0.7f);
            char buf[96];
            std::snprintf(buf, sizeof(buf), "clip: %s   %s", clipNames[clipIdx],
                          animator.isFading()
                              ? (std::string("(cross-fading ") +
                                 std::to_string(static_cast<int>(animator.fadeProgress() * 100)) +
                                 "%)")
                                    .c_str()
                              : "(playing)");
            font.drawText(*renderer, 16.0f, 46.0f, buf, render::Color{0.7f, 1.0f, 0.8f, 1}, 0.5f);
    ...
    MAZ_LOG_INFO("ANIMATOR shutting down (renderer %s)", renderer->isActive() ? "active" : "inactive");
    renderer->shutdown();
    window.shutdown();
    return 0;
}
