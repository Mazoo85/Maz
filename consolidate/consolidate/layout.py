...
#: Stamped into every file this module generates. Nothing without this marker is
#: ever overwritten, which is what makes it safe to point the tool at a
#: repository that already has a README, a CONTRIBUTING, a life of its own.
MARKER = "<!-- generated by maz-consolidate — edits below are overwritten -->"
...
def is_generated(path: Path) -> bool:
    """True when this file is ours to rewrite: absent, or carrying the marker."""
    if not path.exists():
        return True
    try:
...
def render_readme(
    plan: Plan, *, shas: dict[str, str] | None = None, existing: Sequence[str] = ()
) -> str:
    """The front page: what this repo is and what is in it.

    ``existing`` lists the directories an adopted repository already had. They
    are part of what is in here, so leaving them out would make the index of a
    repo like that actively misleading.
    """
    shas = shas or {}
    if plan.adopted:
        lines = [
            ...
            f"was, and repositories folded in from elsewhere land in `{plan.prefix or '.'}/`",
...
def planned_files(
    ...
    """Every generated file and its contents, before anything is written."""
    return {
        plan.index_file: render_readme(plan, shas=shas, existing=existing),
        "CONSOLIDATION.md": render_provenance(plan, shas=shas),
        MANIFEST_NAME: render_manifest(plan, shas=shas),
    }


def conflicts(root: str | Path, plan: Plan) -> list[str]:
    """Generated files that already exist and were not written by us.

    Checked before a build starts, so adopting a repository can never be the
    move that silently eats its README.
    """
    root = Path(root)
    return sorted(
        name
        for name in planned_files(plan)
        if not name.endswith(".json") and not is_generated(root / name)
    )


def write_scaffold(
    root: str | Path, plan: Plan, *, shas: dict[str, str] | None = None, existing: Sequence[str] = ()
) -> list[str]:
    """Write (or refresh) the generated files. Returns the paths written.

    A file that exists without our marker is left exactly as it is — we would
    rather have no index than destroy someone's front page.
    """
    root = Path(root)
    root.mkdir(parents=True, exist_ok=True)
    files = planned_files(plan, shas=shas, existing=existing)
    ...
    for name, text in files.items():
        target = root / name
        if name.endswith(".json") or name == ".gitignore" or is_generated(target):
