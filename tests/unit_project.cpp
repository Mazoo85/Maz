...
int main() {
    ...
    seq.setChannelHighpass(2, 600.0f); // thin the closed hat with a low-cut
    ...
    seq.synth().setSubWaveform(audio::Waveform::StepSine);
    ...
    seq.synth().setOsc3FineTune(-18.0f);
    ...
    seq.synth().setVibratoShape(audio::Waveform::Triangle);
    ...
    seq.addMarker(0, "Intro");    // arrangement markers must round-trip (incl. a spaced name)
    seq.addMarker(8, "Chorus 1");
    // A timeline automation clip: target/bar/track/bars/range/mute + breakpoints must round-trip.
    const int aci = seq.addAutomationClip(static_cast<int>(audio::AutoTarget::SynthCutoff), 4, 2);
    audio::AutomationClip& automClip = seq.automationClip(aci);
    automClip.track = 1;
    automClip.lo = 300.0f;
    ...
    mixer.compressor().setStereoLink(false);
    ...
    mixer.delay().setFeedbackDrive(0.6f);
    ...
    mixer.reverb().setModDepth(2.5f);
    mixer.reverb().setModRate(1.5f);
    ...
    mixer.beatRepeat().setEnabled(true);
    ...
    mixer.multiTapDelay().setFeedback(0.5f);
    ...
    mixer.leveler().setEnabled(true);
    mixer.leveler().setTargetDb(-9.0f);
    mixer.leveler().setResponseMs(600.0f);
    mixer.leveler().setMaxGainDb(15.0f);
    mixer.clipper().setEnabled(true);
    mixer.clipper().setDriveDb(4.5f);
    mixer.clipper().setCeiling(0.75f);
    mixer.clipper().setHardness(0.4f);
    mixer.clipper().setMix(0.7f);
    ...
    mixer.rotary().setMix(0.9f);
    mixer.rotary().setDrive(0.5f);
    ...
    leadVolLane.lfo.sampleHold = true; // random stepped LFO on this lane
    ...
    rmLane.lo = 40.0f;
    rmLane.hi = 1200.0f;
    audio::AutoLane& cmLane = automation.lane(audio::AutoTarget::ChorusMix);
    cmLane.enabled = true;
    cmLane.lfo.shape = audio::Waveform::Sine;
    ...
    rdLane.lo = 0.2f;
    rdLane.hi = 0.75f;
    audio::AutoLane& fsLane = automation.lane(audio::AutoTarget::FreqShift);
    fsLane.enabled = true;
    fsLane.lfo.shape = audio::Waveform::Sine;
    fsLane.lfo.rateHz = 0.35f;
    fsLane.lo = -300.0f;
    fsLane.hi = 450.0f;
    audio::AutoLane& rrLane = automation.lane(audio::AutoTarget::RotaryRate);
    rrLane.enabled = true;
    rrLane.lfo.shape = audio::Waveform::Triangle;
    ...
    dtLane.lo = 60.0f;
    dtLane.hi = 350.0f;
    audio::AutoLane& tdLane = automation.lane(audio::AutoTarget::TremoloDepth);
    tdLane.enabled = true;
    tdLane.lfo.shape = audio::Waveform::Square;
    ...
    check(near(seq2.channelHighpass(2), 600.0f), "channel high-pass (low-cut) round-trips");
    ...
    check(seq2.markerCount() == 2 && seq2.marker(0).bar == 0 && seq2.marker(0).name == "Intro" &&
              seq2.marker(1).bar == 8 && seq2.marker(1).name == "Chorus 1",
          "arrangement markers round-trip (bar + spaced name)");
    check(seq2.automationClipCount() == 1 &&
              seq2.automationClip(0).target == static_cast<int>(audio::AutoTarget::SynthCutoff) &&
    ...
    check(seq2.synth().filterLfoSampleHold(), "filter LFO sample & hold round-trips");
    check(seq2.synth().filterSlope() == 24, "filter slope round-trips");
    check(near(seq2.synth().noiseAttackAmount(), 0.55f) &&
              near(seq2.synth().noiseAttackDecay(), 22.0f),
    ...
    check(seq2.synth().vibratoShape() == audio::Waveform::Triangle, "vibrato LFO shape round-trips");
    ...
    check(seq2.synth().unisonVoices() == 5 && near(seq2.synth().unisonDetune(), 18.0f),
          "unison round-trips");
    check(seq2.synth().subWaveform() == audio::Waveform::StepSine,
          "sub waveform round-trips (incl. the step-sine index)");
    ...
    check(near(seq2.synth().osc3Level(), 0.45f) && near(seq2.synth().osc3Semitones(), -5.0f) &&
              ...
              near(seq2.synth().osc3FineTune(), -18.0f),
    ...
    check(mixer2.filter().enabled() &&
              ...
              near(mixer2.filter().drive(), 0.45f),
    ...
    check(!mixer2.compressor().stereoLink(), "compressor stereo-link round-trips");
    ...
    check(mixer2.delay().enabled() && near(mixer2.delay().time(), 250.0f) &&
              ...
              near(mixer2.delay().duck(), 0.7f) && near(mixer2.delay().feedbackDrive(), 0.6f),
    ...
    check(mixer2.limiter().enabled() && near(mixer2.limiter().inputGainDb(), 6.0f) &&
          ...
          "limiter round-trips");
    check(mixer2.leveler().enabled() && near(mixer2.leveler().targetDb(), -9.0f) &&
              near(mixer2.leveler().responseMs(), 600.0f) && near(mixer2.leveler().maxGainDb(), 15.0f),
          "leveler round-trips");
    ...
    check(mixer2.multiTapDelay().enabled() && near(mixer2.multiTapDelay().timeMs(), 180.0f) &&
              ...
              near(mixer2.multiTapDelay().mix(), 0.45f) && near(mixer2.multiTapDelay().spread(), 0.6f) &&
                  near(mixer2.multiTapDelay().feedback(), 0.5f),
    ...
    check(near(mixer2.reverb().modDepth(), 2.5f) && near(mixer2.reverb().modRate(), 1.5f),
          "reverb tail modulation round-trips");
    ...
    check(leadVol2.enabled && leadVol2.lfo.shape == audio::Waveform::Triangle &&
              ...
              leadVol2.lfo.sampleHold,
          "lead-volume automation lane round-trips (incl. sample & hold)");
    ...
    const audio::AutoLane& cm2 = automation2.lane(audio::AutoTarget::ChorusMix);
    ...
    const audio::AutoLane& fs2 = automation2.lane(audio::AutoTarget::FreqShift);
    check(fs2.enabled && fs2.lfo.shape == audio::Waveform::Sine && near(fs2.lo, -300.0f) &&
              near(fs2.hi, 450.0f),
          "frequency-shift automation lane round-trips");
    const audio::AutoLane& rr2 = automation2.lane(audio::AutoTarget::RotaryRate);
    ...
    const audio::AutoLane& td2 = automation2.lane(audio::AutoTarget::TremoloDepth);
