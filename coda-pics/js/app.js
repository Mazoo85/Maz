...
(function () {
  ...
  function showReadout(spec, size) {
    /* The unknown-words line below says which words were not understood. It does
     * not cover the other way this goes wrong: every word understood, but none of
     * them naming the thing in the picture, so the subject is CODA PICS' own
     * choice. Worth one clause, because that is the difference between a picture
     * of what you asked for and a good picture of something else. */
    var g = spec.grounded;
    var chose = (g && g.parts && !g.subject) ? ' · I picked the subject myself' : '';
    setStatus('<b>' + escapeHtml(PROMPT.describe(spec)) + '</b> · ' +
      size.w + ' × ' + size.h + ' · seed ' + spec.seed + chose);
