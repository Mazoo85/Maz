...
```
open zomboid/index.html         # double-click it, or, from the repo root:
python3 -m http.server          # then visit http://localhost:8000/zomboid/
```

Or play it from the [**MAZ ARCADE hub**](../index.html) — the arcade pill in the
top-left corner takes you back out to every other project.
...
```
zomboid/index.html        # shell + script load order
zomboid/css/style.css     # arcade-cabinet styling, scanlines
zomboid/js/world.js       # Anchorage data: avenues, streets, named buildings
zomboid/js/worldgen.js    # bakes data into a tile grid + loot containers
zomboid/js/items.js       # item definitions + themed loot tables
zomboid/js/audio.js       # WebAudio chiptune SFX + music
zomboid/js/game.js        # engine: states, input, sim, combat, rendering, HUD
```
